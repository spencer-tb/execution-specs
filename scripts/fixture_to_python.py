...
# Regex to strip YAML label syntax: <contract:target:0xADDR> -> 0xADDR
...
def generate_test_file(
    ...
    # -----------------------------------------------------------------------
    # Compute post-state assertions (before assembly, so parametrize can use them)
    # -----------------------------------------------------------------------
    post_code = "    post = {}"
    ...
    if expect_entries and not is_multi:
        # Single-case: extract (d, g, v) from the first fixture key
        first_key = all_keys[0]
        d_idx, g_idx, v_idx = extract_case_indices(first_key)
        result = resolve_expect_for_case(expect_entries, d_idx, g_idx, v_idx)
        if result:
            post_code = generate_post_dict(result, addr_vars)
    elif expect_entries and is_multi:
        # Check if all expect entries share the same result
        ...
        if (all_same or all_wildcard) and all_results:
            post_code = generate_post_dict(all_results[0], addr_vars)
        else:
            # Collect per-case results
            case_results: list[dict | None] = []
            only_storage_differs = True
            for i, case in enumerate(cases_for_fork):
                key = all_keys[i] if i < len(all_keys) else all_keys[0]
                d_i, g_i, v_i = extract_case_indices(key)
                r = resolve_expect_for_case(expect_entries, d_i, g_i, v_i)
                ...
                if r:
                    for a, fields in r.items():
                        ...
                        if non_storage:
                            only_storage_differs = False

            if only_storage_differs and any(case_results):
                # Find contract with storage assertions
                storage_addr = None
                expected_storages: list[dict | None] = []
                for r in case_results:
                    if r is None:
                        expected_storages.append(None)
                        continue
                    found = False
                    for a, fields in r.items():
                        if a.lower() == COINBASE_ADDRESS:
                        ...
                        if "storage" in fields:
                            storage_addr = a
                            expected_storages.append(fields["storage"])
                            found = True
                            break
                    if not found:
                        expected_storages.append(None)

                if storage_addr and any(
                    s is not None for s in expected_storages
                ):
                    storage_var = addr_vars.get(
                        storage_addr.lower(),
                        f'Address("{storage_addr}")',
                    )
                    extra_param_name = "expected_storage"
                    extra_func_param = "    expected_storage: dict,"
                    for s in expected_storages:
                        if s is not None:
                            extra_param_vals.append(
                                _format_storage_from_result(s)
                            )
                        else:
                            extra_param_vals.append("{}")
                    post_code = (
