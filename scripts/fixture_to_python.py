...
def generate_test_file(
    ...
    # Collect all cases for the first fork (Cancun) to detect parametrization.
    all_keys = list(fixture_data.keys())
    first_test = fixture_data[all_keys[0]]

    env = first_test["env"]
    pre = first_test["pre"]
    fork_name = list(first_test["post"].keys())[0]  # e.g. "Cancun"
    ...
    if expect_exception:
        tx_parts.append(f"error={expect_exception}")
    ...
    # Add exception_test marker if transaction is expected to fail
    if needs_tx_exception:
...
def process_single_fixture(
    ...
    try:
        python_code = generate_test_file(fixture_data, filler_path, filler_comment)
