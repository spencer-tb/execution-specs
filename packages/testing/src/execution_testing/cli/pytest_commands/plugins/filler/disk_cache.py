...
def compute_content_hash(
    ...
    h.update(request_data.state.model_dump_json(**model_dump_config).encode())
    # Hash alloc: use state_root when available (free for LazyAlloc),
    # fall back to serialization for genesis Alloc (block 0 only).
    alloc = request_data.input.alloc
    if hasattr(alloc, "_state_root"):
        h.update(bytes(alloc._state_root))
    else:
        h.update(
            json.dumps(
                alloc.model_dump(mode="json", **model_dump_config),
                sort_keys=True,
                separators=(",", ":"),
            ).encode()
        )
    h.update(
        request_data.input.env.model_dump_json(**model_dump_config).encode()
    )
    for tx in request_data.input.txs:
        h.update(tx.model_dump_json(**model_dump_config).encode())
    if request_data.input.blob_params is not None:
        h.update(
            request_data.input.blob_params.model_dump_json(
                **model_dump_config
            ).encode()
        )
    h.update(b"\x01" if state_test else b"\x00")
    return h.hexdigest()
...
def deserialize_output(
    ...
    alloc_raw = data["alloc"]
    if isinstance(alloc_raw, str):
        alloc = LazyAllocStr(raw=alloc_raw, _state_root=result.state_root)
    else:
        alloc = LazyAllocStr(
            raw=json.dumps(
                alloc_raw,
