...
def transition_fork_criteria(
    ...
    Timestamp transitions take precedence, as `ByTimestamp` sorts after
    every `ByBlockNumber` in the spec.
    """
...
class T8N(Load):
    ...
    def _run_blockchain_test(self, block_env: Any, block_output: Any) -> None:
        ...
        if self.fork.has_zero_nonce_storage_accounts and self.is_fork_block:
