...
class Amsterdam(BPO2):
    ...
    def cost_per_state_byte(
        ...
        raw = (gas_limit * BLOCKS_PER_YEAR + 2 * TARGET - 1) // (
            2 * TARGET
        )
        shifted = raw + OFFSET
    ...
    def gas_costs(
        ...
        return replace(
            parent,
            G_STORAGE_SET=(
                parent.G_STORAGE_UPDATE
                - parent.G_COLD_SLOAD
                + STATE_BYTES_PER_STORAGE_SET * cpsb
            ),
            G_NEW_ACCOUNT=new_acct,
            G_CREATE=REGULAR_GAS_CREATE + new_acct,
            G_TRANSACTION_CREATE=(REGULAR_GAS_CREATE + new_acct),
            G_AUTHORIZATION=(
                PER_AUTH_BASE_COST
                + (
            ...
            R_AUTHORIZATION_EXISTING_AUTHORITY=new_acct,
        )
    ...
    def _calculate_sstore_gas(
        ...
        gas_cost = (
            0 if metadata["key_warm"] else gas_costs.G_COLD_SLOAD
        )
