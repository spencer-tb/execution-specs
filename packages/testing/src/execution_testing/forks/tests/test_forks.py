...
def test_execution_witness_implicit_code_addresses() -> None:
    """Implicit witness code addresses accumulate per fork via EIPs."""
    assert Shanghai.execution_witness_implicit_code_addresses() == []

    cancun = {
        address.hex()
        for address in Cancun.execution_witness_implicit_code_addresses()
    }
    assert cancun == {"0x000f3df6d732807ef1319fb7b8bb8522d0beac02"}

    prague = {
        address.hex()
        for address in Prague.execution_witness_implicit_code_addresses()
    }
    ...
    assert amsterdam == prague | {
        "0x0000bff46984e3725691fa540a8c7589300d8282",
        "0x000064d678505ad48f8ccb093bc65613800e8282",
    }
    assert "0x00000000219ab540356cbb839cbe05303d7705fa" not in amsterdam
