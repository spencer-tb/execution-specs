...
def build_refund_tx(
    ...
    # EIP-8037: remaining state gas = intrinsic state gas - state gas returned
    # to reservoir for existing authorities
    remaining_state_gas = auth_state_gas - auth_state_refund

    # In the spec, the refund cap uses tx_gas_used_before_refund which is
    # tx.gas - gas_left - state_gas_left (combined regular + remaining state).
...
def test_multi_transaction_gas_accounting(
    ...
    # Extra tx has no state gas (no auth), so its state gas contribution = 0
    block_regular = refund_tx_block_regular + extra_tx_intrinsic_gas_cost
    block_state = tx_state_gas  # extra_tx contributes 0 state gas
