...
def test_block_gas_used_mixed_txs(
    ...
    block_regular = (
        num_stop * intrinsic_gas + num_sstore * tx_regular_sstore
    )
...
def test_block_gas_refund_eip7778_no_block_reduction(
    ...
    expected = max(
        num_txs * tx_regular, num_txs * sstore_state_gas
    )
    txs = []
...
def test_block_gas_used_create_tx(
    ...
    expected = max(
        create_regular + stop_regular, create_state_gas
    )
