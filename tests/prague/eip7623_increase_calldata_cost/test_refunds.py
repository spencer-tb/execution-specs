...
def state_gas_refund(fork: Fork, refund_type: RefundType) -> int:
    ...
    if fork >= Amsterdam and RefundType.AUTHORIZATION_EXISTING_AUTHORITY in refund_type:
        gas_costs = fork.gas_costs()
        return gas_costs.R_AUTHORIZATION_EXISTING_AUTHORITY
    return 0
...
def max_refund(fork: Fork, refund_type: RefundType) -> int:
    ...
    if fork < Amsterdam and RefundType.AUTHORIZATION_EXISTING_AUTHORITY in refund_type:
        max_refund += gas_costs.R_AUTHORIZATION_EXISTING_AUTHORITY
    return max_refund
...
def to(
    ...
    setup = Op.PUSH2(0)
    loop_body = (
        Op.JUMPDEST + Op.PUSH1(1) + Op.SWAP1 + Op.SUB
        + Op.DUP1 + Op.PUSH1(loop_target) + Op.JUMPI
    )
    teardown = Op.POP
