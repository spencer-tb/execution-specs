...
def test_gas_diff_pointer_vs_direct_call(
    ...
    Redundant from Amsterdam: EIP-8037 replaces the one-dimensional SSTORE gas cost
    (G_STORAGE_SET) with a two-dimensional split: regular gas (GAS_STORAGE_UPDATE -
    GAS_COLD_SLOAD) and state gas (STATE_BYTES_PER_STORAGE_SET *
    cost_per_state_byte). In sub-calls state_gas_left=0, so state gas falls to
    gas_left — changing what the GAS opcode reports. Auth refund
    (STATE_BYTES_PER_NEW_ACCOUNT * cost_per_state_byte) goes to
    state_gas_reservoir, further altering gas visibility between frames.

    TODO: Add Amsterdam-specific variant in tests/amsterdam/ that verifies pointer
    vs direct call gas costs under EIP-8037's 2D gas model with reservoir semantics.
    """
    env = Environment()
...
def test_pointer_call_followed_by_direct_call(
    ...
    Redundant from Amsterdam: EIP-8037 replaces one-dimensional SSTORE gas costs
    with a 2D split (regular + state gas), changing what the GAS opcode reports.
    See test_gas_diff_pointer_vs_direct_call for details.

    TODO: Add Amsterdam-specific variant in tests/amsterdam/ that verifies pointer
    warming behavior with 2D gas cost measurements.
    """
    env = Environment()
