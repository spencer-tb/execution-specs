...
def test_set_code_to_precompile_not_enough_gas_for_precompile_execution(
    ...
    Redundant from Amsterdam: EIP-8037 replaces the one-dimensional gas model this
    test verifies. Auth intrinsic cost becomes (STATE_BYTES_PER_AUTH_BASE +
    STATE_BYTES_PER_NEW_ACCOUNT) * cost_per_state_byte per auth (state gas), plus
    PER_AUTH_BASE_COST (regular gas). Auth refund for existing accounts goes to
    state_gas_reservoir instead of refund_counter, making the discount calculation
    (PER_EMPTY_ACCOUNT_COST - PER_AUTH_BASE_COST) and receipt gas expectation invalid.

    TODO: Add Amsterdam-specific variant in tests/amsterdam/ that verifies receipt
    gas and auth refund behavior under EIP-8037's 2D gas model.
    """
