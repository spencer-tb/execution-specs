"""
Unit test proving the PBT-mode block-execution path rejects a
block whose header claims a `state_root` that does not match the
tree-computed root.

Fixture tests only *record* this expectation: fill-time
verification is skipped whenever `rlp_modifier` is set
(`BlockchainTest.generate_block_data`), and no client consumes
PBT-mode fixtures either, so nothing exercises the check end to
end today. This test drives it directly, through
`ethereum.forks.amsterdam.fork.execute_block`, whose
...
from ethereum.forks.amsterdam.vm.gas import calculate_excess_blob_gas
...
def test_execute_block_rejects_a_tampered_state_root() -> None:
    ...
    # `execute_block` annotates `pre_state` with the MPT provider, but
    # its body only touches the commitment-neutral `PreState` protocol
    # surface, which the PBT provider implements too.
    execute_block(block, pre_state, chain_context)  # type: ignore[arg-type]
