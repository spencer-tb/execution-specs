...
from ethereum.partitioned_binary_tree import (
    EMPTY_TRIE_ROOT as PBT_EMPTY_TRIE_ROOT,
)
from ethereum.partitioned_binary_tree import (
...
def test_get_code_contract_is_identical_across_providers() -> None:
    """
    Both providers implement the `PreState.get_code` error contract
    the same way: empty code for `EMPTY_CODE_HASH`, and the typed
    `UnknownCodeHashError` for any other hash with no stored bytecode.
    """
    for state in (MptState(), PbtState()):
        assert state.get_code(EMPTY_CODE_HASH) == b""
        with pytest.raises(UnknownCodeHashError):
            state.get_code(keccak256(b"never stored"))
