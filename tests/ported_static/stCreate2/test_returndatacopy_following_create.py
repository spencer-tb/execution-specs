...
def test_returndatacopy_following_create(
    ...
    post = {
        callee: Account(
            storage={0: 1},
            code=Op.PUSH1[0x0] + Op.PUSH1[0x28] + Op.CODECOPY(dest_offset=0x0, offset=0x1f, size=Op.DUP1) + Op.PUSH1[0x0] + Op.PUSH1[0x0] + Op.POP(Op.CREATE2) + Op.RETURNDATACOPY(dest_offset=0x0, offset=0x0, size=0x20) + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x0)) + Op.STOP + Op.INVALID + Op.MSTORE(offset=0x0, value=0x111122223333444455556666777788889999aaaabbbbccccddddeeeeffff) + Op.RETURN(offset=0x0, size=0x20) + Op.STOP,
        ),
        contract: Account(
            code=Op.CALL(gas=Op.GAS, address=Op.CALLDATALOAD(offset=0x0), value=0x0, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0) + Op.STOP,
        ),
        callee_1: Account(
            storage={0: 1},
            code=Op.PUSH1[0x0] + Op.PUSH1[0x2] + Op.CODECOPY(dest_offset=0x0, offset=0x1f, size=Op.DUP1) + Op.PUSH1[0x0] + Op.PUSH1[0x0] + Op.POP(Op.CREATE2) + Op.RETURNDATACOPY(dest_offset=0x0, offset=0x0, size=0x20) + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x0)) + Op.STOP + Op.INVALID + Op.STOP + Op.STOP,
        ),
    }
