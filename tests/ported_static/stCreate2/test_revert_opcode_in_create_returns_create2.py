...
def test_revert_opcode_in_create_returns_create2(
    ...
    post = {
        contract: Account(
            storage={0: 32},
            code=Op.PUSH1[0x0] + Op.PUSH1[0xe] + Op.CODECOPY(dest_offset=0x0, offset=0x17, size=Op.DUP1) + Op.PUSH1[0x0] + Op.PUSH1[0x0] + Op.POP(Op.CREATE2) + Op.SSTORE(key=0x0, value=Op.RETURNDATASIZE) + Op.STOP + Op.STOP + Op.INVALID + Op.MSTORE(offset=0x0, value=0x112233) + Op.REVERT(offset=0x0, size=0x20) + Op.STOP + Op.STOP,
        ),
    }
