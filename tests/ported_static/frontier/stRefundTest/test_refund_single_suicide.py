"""
Verify gas accounting when a contract calls itself and the inner frame
self-destructs with the contract as its own beneficiary: the balance
...
as a minimal calldata-dispatched self-call; the sender balance, refund cap
and budget derive from fork composites and the post branches on EIP-6780
for the contract's survival.
"""
...
GAS_PRICE = 10
FLAG_SLOT = 0xA
# Any non-empty calldata selects the outer (self-call) path.
...
def test_refund_single_suicide(
    ...
    """Self-destruct to self in an inner frame; derive the gas accounting."""
    # The dispatch runs in both frames: taken with calldata (outer), not
    # taken without (inner). Both directions cost the same.
    ...
    body_cost = (
        2 * dispatch_code.gas_cost(fork)
        + kill_code.gas_cost(fork)
