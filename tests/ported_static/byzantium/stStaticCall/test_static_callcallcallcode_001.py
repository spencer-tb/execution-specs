"""
Verify the static flag propagates through a DELEGATECALL made inside a
static context: the delegated probe's SSTOREs are still violations and
halt only the delegated frame, while a memory-only probe succeeds;
either way the enclosing frames complete and record success.
...
def test_static_callcallcallcode_001(
    ...
    """Enforce the static flag across a nested DELEGATECALL."""
    if probe_type == "sstore_probes":
        # Delegated into a static context, every SSTORE is a violation:
        # the delegated frame halts and none of these values persist.
    ...
    delegator = pre.deploy_contract(
        code=Op.MSTORE(offset=0x3, value=0x1)
        + Op.POP(Op.DELEGATECALL(address=probe, args_size=0x40, ret_size=0x40))
    ...
    depth2 = pre.deploy_contract(
        code=Op.MSTORE(offset=0x3, value=0x1)
        + Op.POP(
            Op.STATICCALL(address=delegator, args_size=0x40, ret_size=0x40)
        )
        + Op.MSTORE(offset=0x3, value=0x1)
        + Op.STOP,
    )
    ...
    # The delegated frame's failure is invisible one level up: the
    # static frames complete either way, so the writable frames record
    # success.
    post = {
        probe: Account(storage={}),
        delegator: Account(storage={}),
