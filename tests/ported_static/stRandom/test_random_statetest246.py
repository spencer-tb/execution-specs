...
def test_random_statetest246(
    ...
    pre[contract] = Account(
        ...
        nonce=0,
        code=(
        Op.NUMBER + Op.TIMESTAMP + Op.BALANCE(address=Op.COINBASE) + Op.PREVRANDAO
        + Op.COINBASE + Op.NUMBER + Op.TIMESTAMP + Op.SWAP4
    ...
    post = {
        contract: Account(
            storage={57403: 1000},
            code=Op.NUMBER + Op.TIMESTAMP + Op.BALANCE(address=Op.COINBASE) + Op.PREVRANDAO + Op.COINBASE + Op.NUMBER + Op.TIMESTAMP + Op.SWAP4 + Op.SSTORE(key=Op.MULMOD, value=Op.OR),
        ),
        coinbase: Account(
