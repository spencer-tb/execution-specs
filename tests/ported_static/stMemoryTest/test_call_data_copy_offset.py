...
def test_call_data_copy_offset(
    ...
    post = {
        contract: Account(
            code=Op.MSTORE(offset=0x0, value=0x123456789abcdef) + Op.CALL(gas=0xffff, address=0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee, value=Op.DUP1, args_offset=Op.DUP2, args_size=0xf, ret_offset=Op.DUP1, ret_size=0x0) + Op.STOP,
        ),
        callee: Account(
            storage={0: 0xffffffffffffffffffffffffffffffff},
            code=Op.MSTORE(offset=0x0, value=0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff) + Op.CALLDATACOPY(dest_offset=0x0, offset=0xffff, size=0x10) + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x0)) + Op.STOP,
        ),
    }
