...
def test_call_recursive_bomb_pre_call(
    ...
    pre[callee] = Account(
        ...
        + Op.SSTORE(key=0x1, value=Op.CALL(gas=Op.SUB(Op.GAS, 0x36b00), address=Op.ADDRESS, value=0x0, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0))
        + Op.STOP
    ),
    )
    ...
    post = {
        callee: Account(
            storage={0: 1024, 1: 1},
            code=Op.SSTORE(key=0x0, value=Op.ADD(Op.SLOAD(key=0x0), 0x1)) + Op.SSTORE(key=0x1, value=Op.CALL(gas=Op.SUB(Op.GAS, 0x36b00), address=Op.ADDRESS, value=0x0, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)) + Op.STOP,
        ),
        contract: Account(
            code=Op.POP(Op.CALL(gas=0x186a0, address=0xbad304eb96065b2a98b57a48a06ae28d285a71b5, value=0x17, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)) + Op.CALL(gas=0x7ffffffffffffff, address=0x1b3f200856856edc2e98efcd637775c6e341e3c0, value=0x17, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0) + Op.STOP,
        ),
    }
