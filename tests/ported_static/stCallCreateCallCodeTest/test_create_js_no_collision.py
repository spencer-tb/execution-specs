...
@pytest.mark.ported_from(
    [
        "tests/static/state_tests/stCallCreateCallCodeTest/createJS_NoCollisionFiller.json",  # noqa: E501
    ],
)
    ...
    post = {
        Address("0x6295ee1b4f6dd65047762f924ecd367c17eabf8f"): Account(
            ...
            code=(
                ...
                + Op.DIV
                + Op.JUMPI(
                    pc=Op.PUSH2[0x42], condition=Op.EQ(0x43D726D6, Op.DUP1)
                )
                + Op.JUMPI(
                    pc=Op.PUSH2[0x50], condition=Op.EQ(0x91B7F5ED, Op.DUP1)
                )
                + Op.JUMPI(
                    pc=Op.PUSH2[0x61], condition=Op.EQ(0xD686F9EE, Op.DUP1)
                )
                + Op.JUMPI(
                    pc=Op.PUSH2[0x6F], condition=Op.EQ(0xF5BADE66, Op.DUP1)
                )
                + Op.JUMPI(
                    pc=Op.PUSH2[0x80], condition=Op.EQ(0xFCFFF16F, Op.DUP1)
                )
                + Op.STOP
                + Op.JUMPDEST
                + Op.PUSH2[0x4A]
                ...
                + Op.JUMPI(
                    ...
                    condition=Op.EQ(
                        Op.AND(Op.SUB(Op.EXP(0x2, 0xA0), 0x1), Op.CALLER),
                        Op.AND(
                            Op.SUB(Op.EXP(0x2, 0xA0), 0x1), Op.SLOAD(key=0x3)
                        ),
                    ),
                )
                + Op.JUMP(pc=0x300)
                ...
                + Op.MUL(
                    Op.SLOAD(key=0x2),
                    Op.SUB(Op.TIMESTAMP, Op.SLOAD(key=0x5)),
                )
                + Op.SWAP1
                + Op.POP
                + Op.JUMPI(
                    pc=0x2C7, condition=Op.GT(Op.DUP2, Op.SLOAD(key=0x1))
                )
                + Op.AND(Op.SUB(Op.EXP(0x2, 0xA0), 0x1), Op.SLOAD(key=0x3))
                + Op.PUSH1[0x0]
                ...
                + Op.POP
                + Op.JUMPI(
                    pc=0x345, condition=Op.GT(Op.DUP2, Op.SLOAD(key=0x1))
                )
                + Op.JUMP(pc=0x37D)
                + Op.JUMPDEST
                + Op.AND(Op.SUB(Op.EXP(0x2, 0xA0), 0x1), Op.SLOAD(key=0x0))
