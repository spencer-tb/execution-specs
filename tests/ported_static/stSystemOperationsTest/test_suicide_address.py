...
def test_suicide_address(
    ...
    pre[contract] = Account(
        ...
        nonce=0,
        code=(
        Op.SSTORE(key=0x0, value=Op.ADDRESS) + Op.SELFDESTRUCT(address=Op.ADDRESS)
        + Op.STOP
    ),
    )
    ...
    post = {
        contract: Account(
            ...
            code=Op.SSTORE(key=0x0, value=Op.ADDRESS) + Op.SELFDESTRUCT(address=Op.ADDRESS) + Op.STOP,
        ),
    }
