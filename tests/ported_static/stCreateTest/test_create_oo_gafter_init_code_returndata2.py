...
@pytest.mark.parametrize(
    ...
    [
        ...
        (95000, {Address("0xb94f5374fce5edbc8e2a8697c15331677e6ebf0b"): Account(storage={2: 0x6460016001556000526005601bf3}, code=Op.MSTORE(offset=0x0, value=0x6460016001556000526005601bf3) + Op.POP(Op.CREATE(value=0x0, offset=0x12, size=0xe)) + Op.SSTORE(key=0x1, value=Op.RETURNDATASIZE) + Op.RETURNDATACOPY(dest_offset=0x0, offset=0x0, size=0x0) + Op.SSTORE(key=0x2, value=Op.MLOAD(offset=0x0)) + Op.STOP), Address("0xf1ecf98489fa9ed60a664fc4998db699cfa39d40"): Account(code=Op.SSTORE(key=0x1, value=0x1))}),
    ],
    ids=['case0', 'case1'],
)
