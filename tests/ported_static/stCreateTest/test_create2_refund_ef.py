...
."""
    ...
    post = {
        callee: Account(storage={0: 1}, code=Op.SSTORE(key=Op.DUP1, value=0x0) + Op.STOP),
        contract: Account(
            code=Op.PUSH1[0x0] + Op.PUSH1[0x19] + Op.CODECOPY(dest_offset=Op.DUP4, offset=0x11, size=Op.DUP1) + Op.DUP2 + Op.DUP1 + Op.SSTORE(key=0x0, value=Op.CREATE2) + Op.STOP + Op.INVALID + Op.POP(Op.CALL(gas=0xc350, address=0x5ef94d, value=Op.DUP1, args_offset=Op.DUP1, args_size=Op.DUP1, ret_offset=Op.DUP1, ret_size=0x0)) + Op.MSTORE8(offset=0x0, value=0xef) + Op.RETURN(offset=0x0, size=0x1),
        ),
    }
