...
def test_gas_cost_return(
    ...
    pre[contract] = Account(
        ...
        nonce=0,
        code=(
        Op.MSTORE(offset=0x0, value=0x60a7) + Op.MSTORE(offset=0x20, value=0x60a7)
        + Op.MSTORE(offset=0x40, value=0x60a7) + Op.MSTORE(offset=0x0, value=Op.GAS)
        ...
        + Op.SSTORE(key=0x0, value=Op.SUB(Op.MLOAD(offset=0x20), Op.MLOAD(offset=0x40)))
        + Op.STOP
    ),
        storage={0x0: 0x60a7},
    )
    pre[callee] = Account(balance=0xba1a9ce0ba1a9ce, nonce=0, code=Op.RETURN(offset=0xff, size=0x0))
    ...
    post = {
        contract: Account(
            code=Op.MSTORE(offset=0x0, value=0x60a7) + Op.MSTORE(offset=0x20, value=0x60a7) + Op.MSTORE(offset=0x40, value=0x60a7) + Op.MSTORE(offset=0x0, value=Op.GAS) + Op.POP(Op.CALL(gas=0x10000, address=0x1000, value=0x0, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)) + Op.MSTORE(offset=0x20, value=Op.SUB(Op.MLOAD(offset=0x0), Op.GAS)) + Op.MSTORE(offset=0x0, value=Op.GAS) + Op.POP(Op.CALL(gas=0x10000, address=0x2000, value=0x0, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)) + Op.MSTORE(offset=0x40, value=Op.SUB(Op.MLOAD(offset=0x0), Op.GAS)) + Op.SSTORE(key=0x0, value=Op.SUB(Op.MLOAD(offset=0x20), Op.MLOAD(offset=0x40))) + Op.STOP,
        ),
