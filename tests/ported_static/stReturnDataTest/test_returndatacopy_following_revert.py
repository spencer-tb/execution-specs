...
def test_returndatacopy_following_revert(
    ...
    pre[contract] = Account(
        ...
        nonce=0,
        code=(
        Op.POP(Op.CALL(gas=0x900000000, address=0x2159735ba26480adc67f0ee9d4a05e5405a5cf83, value=0x0, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0))
    ...
    post = {
        callee: Account(
            code=Op.MSTORE(offset=0x0, value=0x111122223333444455556666777788889999aaaabbbbccccddddeeeeffff) + Op.REVERT(offset=0x0, size=0x20) + Op.STOP,
        ),
        contract: Account(
            storage={0: 0x111122223333444455556666777788889999aaaabbbbccccddddeeeeffff},
            code=Op.POP(Op.CALL(gas=0x900000000, address=0x2159735ba26480adc67f0ee9d4a05e5405a5cf83, value=0x0, args_offset=0x0, args_size=0x0, ret_offset=0x0, ret_size=0x0)) + Op.RETURNDATACOPY(dest_offset=0x0, offset=0x0, size=0x20) + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x0)) + Op.STOP,
        ),
    }
