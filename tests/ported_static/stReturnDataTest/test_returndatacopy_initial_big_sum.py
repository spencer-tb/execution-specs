...
def test_returndatacopy_initial_big_sum(
    ...
    pre[contract] = Account(
        ...
        + Op.RETURNDATACOPY(dest_offset=0x0, offset=Op.EXP(0x2, 0x3f), size=Op.EXP(0x2, 0x3f))
    ...
    post = {
        contract: Account(
            storage={0: 1},
            code=Op.MSTORE(offset=0x0, value=0x112233445566778899aabbccddeeff) + Op.RETURNDATACOPY(dest_offset=0x0, offset=Op.EXP(0x2, 0x3f), size=Op.EXP(0x2, 0x3f)) + Op.SSTORE(key=0x0, value=Op.MLOAD(offset=0x0)) + Op.STOP,
        ),
    }
