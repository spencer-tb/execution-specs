...
def test_sstore_call_to_self_sub_refund_below_zero(
    ...
    post = {
        contract: Account(
            storage={1: 3},
            code=Op.JUMPI(pc=0x15, condition=Op.EQ(Op.ADDRESS, Op.CALLER)) + Op.SSTORE(key=0x1, value=0x0) + Op.CALL(gas=Op.GAS, address=Op.ADDRESS, value=Op.DUP1, args_offset=Op.DUP1, args_size=Op.DUP1, ret_offset=Op.DUP1, ret_size=0x0) + Op.STOP + Op.JUMPDEST + Op.SSTORE(key=0x1, value=0x3) + Op.STOP,
        ),
    }
