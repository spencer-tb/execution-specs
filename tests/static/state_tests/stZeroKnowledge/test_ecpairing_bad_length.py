...
Ported from:
tests/static/state_tests/stZeroKnowledge/ecpairing_bad_length_191Filler.json
...
def test_ecpairing_bad_length_191(
    ...
    pre[coinbase] = Account(balance=0x1258db, nonce=0)
    ...
    pre[sender] = Account(balance=0xde0b6b3a751a725, nonce=7)
    ...
    tx = Transaction(
        ...
        to=contract,
        data=bytes.fromhex(""),
    ...
    post = {
        ...
        sender: Account(storage={}, nonce=8, code=b""),
        callee_14: Account(storage={}, nonce=0, balance=0xde0b6b3a7640000, code=b""),
        contract: Account(
            storage={0: 0xb10e2d527612073b26eecdfd717e6a320cf44b4afac2b0732d9fcbe2b7fa0cf6},
            nonce=1,
            balance=0,
...
@pytest.mark.ported_from(
    ["tests/static/state_tests/stZeroKnowledge/ecpairing_bad_length_193Filler.json"],
)
@pytest.mark.valid_from("Cancun")
@pytest.mark.parametrize(
    "tx_gas_limit",
...
@pytest.mark.pre_alloc_mutable
def test_ecpairing_bad_length_193(
    state_test: StateTestFiller,
    pre: Alloc,
    tx_gas_limit: int,
) -> None:
    ...
    pre[callee_9] = Account(balance=0xde0b6b3a7640000, nonce=0)
    pre[callee_10] = Account(balance=0xde0b6b3a7640000, nonce=0)
    pre[coinbase] = Account(balance=0x160483, nonce=0)
    pre[callee_11] = Account(balance=0xde0b6b3a7640000, nonce=0)
    pre[callee_12] = Account(balance=0xde0b6b3a7640000, nonce=0)
    pre[callee_13] = Account(balance=0xde0b6b3a7640000, nonce=0)
    pre[sender] = Account(balance=0xde0b6b3a74dfb7d, nonce=8)
    pre[callee_14] = Account(balance=0xde0b6b3a7640000, nonce=0)
    pre[contract] = Account(
        balance=0,
        nonce=1,
    ...
    tx = Transaction(
        ...
        to=contract,
        data=bytes.fromhex(""),
        gas_limit=tx_gas_limit,
        gas_price=10,
        nonce=8,
        value=0,
