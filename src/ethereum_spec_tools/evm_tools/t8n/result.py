...
def build_result(
    ...
    try:
        ...
        # The active commitment cannot encode the resulting state (see
        # `PreState.compute_state_root`). Such a block is rejected and
        # leaves the chain state unchanged, so report the pre-state
        # root.
