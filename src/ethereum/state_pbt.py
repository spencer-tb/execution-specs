...
maps them to tree keys and values and [`compute_state_root`] commits
the result, both through [`ethereum.partitioned_binary_tree`].
...
[`ethereum.partitioned_binary_tree`]: ref:ethereum.partitioned_binary_tree
[psw]: ref:ethereum.partitioned_binary_tree.state_write
"""
...
from ethereum.partitioned_binary_tree import root as binary_tree_root
...
def embed_flat_state(
    ...
    have no place in the tree. [`_apply_diff_to_trie`] holds to the
    same rule when it reaches the tree incrementally.

    [`_apply_diff_to_trie`]: ref:ethereum.state_pbt._apply_diff_to_trie
    """
    trie = BinaryTrie()
...
class State:
    ...
    def compute_state_root(self, block_diff: BlockDiff) -> Root:
        ...
        tree, via [`_apply_diff_to_trie`], as explicit insertions,
        ...
        _apply_diff_to_trie(trie, self, block_diff)
        return binary_tree_root(trie)


def _apply_diff_to_trie(
